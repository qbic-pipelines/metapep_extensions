...
def main(args=None):
    ...
    # Splits list in blovks of size n
    def chunks(lst, n):
    ...
    for attempt in range(3):
        try:
            all_records = []
            for taxIdsBatch in chunks(taxIds, 50):
                ids_str = ",".join(taxIdsBatch)
                with Entrez.efetch(db="taxonomy", id=ids_str) as entrez_handle:
                    batch_records = Entrez.read(entrez_handle)
                    all_records.extend(batch_records)
                time.sleep(1)

            if len(all_records) == len(taxIds):
                for taxid, rec in zip(taxIds, all_records):
                    rank = rec["Rank"]
                    if rank != "strain":
    ...
    for attempt in range(3):
        try:
            with Entrez.elink(
                dbfrom="taxonomy", db="assembly", LinkName="taxonomy_assembly", id=list(taxIds_without_assemblyID)
            ) as entrez_handle:
    ...
    def entrez_read_with_retry(handle, max_attempts=3, delay=5):
        for attempt in range(max_attempts):
            try:
                ...
                print(f"[ERROR] Unexpected Entrez.read() error: {e}")
                time.sleep(delay)
        print("Entrez.read() failed after multiple retries.")
        return None  # so script can continue
