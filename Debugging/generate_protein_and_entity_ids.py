...
def main(args=None):
    ...
    with gzip.open(args.out_proteins, "wt") as outfile_proteins, open(
        ...
        if args.entrez_proteins:
            ...
            keys_ep = set(entities_proteins["protein_id"])
            keys_p  = set(proteins       ["protein_id"])
            if keys_ep != keys_p:
                missing_in_p  = keys_ep - keys_p
                missing_in_ep = keys_p  - keys_ep
                err  = []
                if missing_in_p:
                    err.append(f"protein_id(s) in entities_proteins not in proteins: {sorted(missing_in_p)}")
                if missing_in_ep:
                    err.append(f"protein_id(s) in proteins not in entities_proteins: {sorted(missing_in_ep)}")
                raise RuntimeError("Merge key mismatch:\n  " + "\n  ".join(err))
            else:
